...
addEventListener('click', function (event) {
  ...
  for (var _i = 0; _i < particleCount; _i++) {
    particles.push(new Particles(mouse.x, mouse.y, 5, "hsl(".concat(Math.random() * 360, ", 50%, 50%)"), {
      x: Math.cos(angleIncrement * _i) * Math.random() * power,
      y: Math.sin(angleIncrement * _i) * Math.random() * power
    }));
  }
});
init();
